            ...
            case 0: // Array
                ...
                if ($this->SubType === 0) {
                    ...
                    $x = str_replace(array(
                        ...
                        '->',
                        '['), $z, $x);
                    if (mb_substr($x, strlen($x) - 1, 1) === ']') $x = mb_substr($x, 0, strlen($x) - 1);
                    $ItemLst = explode($z, $x);
                    $ItemNbr = count($ItemLst);
                    $Item0 = &$ItemLst[0];
                    // Check first item
                    if ($Item0[0] === '~') {
                        $Item0 = mb_substr($Item0, 1);
            ...
            foreach ($Loc->OpeAct as $i => $ope) {
                switch ($ope) {
                    ...
                    case 2:
                        ...
                        break;
                    case 3:
                        if (strlen('' . $CurrVal) > $Loc->OpePrm[$i]) $CurrVal = (($Loc->OpeUtf8) ? mb_substr('' . $CurrVal, 0, $Loc->OpePrm[$i], 'UTF-8') : mb_substr('' . $CurrVal, 0, $Loc->OpePrm[$i])) . $Loc->OpeEnd;
                        break;
                    case 5:
        ...
        if ($Loc->ConvMode === 1) { // Usual string conversion
            ...
            if ($Loc->ConvWS) {
                ...
                do {
                    ...
                    if ($pos !== false) $CurrVal = mb_substr_replace($CurrVal, $nbsp, $pos, 1);
                } while ($pos !== false);
            }
            if ($Loc->ConvJS) {
